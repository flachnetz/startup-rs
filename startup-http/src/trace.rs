...
use opentelemetry::trace::{FutureExt, OrderMap, Status};
        ...
        S: Clone,
{
    ...
    fn call(&mut self, req: Request<B>) -> Self::Future {
        ...
        attributes.insert(HTTP_METHOD, Value::String(http_method_str(req.method()).into()));
        attributes.insert(HTTP_FLAVOR, Value::String(http_flavor(req.version()).into()));
        attributes.insert(HTTP_URL, Value::String(uri.to_string().into()));

        if let Some(path) = uri.path_and_query() {
            attributes.insert(HTTP_TARGET, Value::String(path.as_str().to_owned().into()));
        }
        if let Some(user_agent) = req.headers().get(header::USER_AGENT).and_then(|s| s.to_str().ok()) {
            attributes.insert(HTTP_USER_AGENT, Value::String(user_agent.to_owned().into()));
        }

        builder.attributes = Some(attributes.into());
        ...
        let fut = x.with_context(cx.clone()).map(
            move |res: Result<Response<BoxBody>, <S as tower_service::Service<Request<B>>>::Error>| {
                match res {
                    ...
                    Err(error) => {
                        let span = cx.span();
                        span.set_status(Status::Error { description: format!("{:?}", error).into() });
